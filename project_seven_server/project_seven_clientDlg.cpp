...
BOOL CprojectsevenclientDlg::OnInitDialog()
{
	...
	AfxGetMainWnd()->SetWindowText(_T("计算机172 陈屹宙 陈奕楠"));
	// TODO: 在此添加额外的初始化代码
	WSADATA wsaData;
...
void CprojectsevenclientDlg::OnClickedConnect()
{
	...
	/*
	ofile.open("D:\\BUFF.txt", ios::binary);
	while (1) {
		 接收信息并显示 
		...
		if (check == "over") {
			sendto(c_Socket, "over", strlen("over"), 0, (LPSOCKADDR)&d_addr, addrlen);
			TRACE("rec over2!\n");
			AfxMessageBox(_T("传输完成 缓存文件存放于D:\\BUFF.txt,请重定向文件转存"));
			break;
		}
...
{
	// TODO: 在此添加控件通知处理程序代码
	AfxMessageBox(_T("计算机172 陈屹宙 陈奕楠"));
}
...
void CprojectsevenclientDlg::OnClickedListen()
{
	...
	/*
	cmp_daddr.sin_family = AF_INET;
	USES_CONVERSION;
	cmp_daddr.sin_addr.S_un.S_addr = inet_addr("127.0.0.1");
	cmp_daddr.sin_port = htons(6001);
	*/
	SOCKET c_Socket = socket(AF_INET, SOCK_DGRAM, 0);
	if (SOCKET_ERROR == c_Socket)
	{
